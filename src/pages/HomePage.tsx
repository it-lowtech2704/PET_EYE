...
const DEALS = [
    {
        ...
        image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDCZuFoVfosq9FXZ6QK-CbMHgVtQ7s7Fa1yMaYzVnaufm3eIMQRr-cwwnb-9jDDJOrNqMIT2osm5yQ0Rl7lBrQ0R1YWn--WpQtMvUBTrqmCS71v9Me-9XqDxmt7poSwbIdEx40IYc-AWUE6WWC8x97_ZY4jTRS-z1n3xI-8snVgPG3B-55A3OZBCiwDhmSUsb3CeNL7Unsqe2X3BIw2vD9ekD100_JE06YEDWMvfVK-7aXTr__bky95rhuer8xDpq9BfM5jroIaaHIM',
        alt: 'Cat Food',
        category: 'Thức ăn mèo',
        name: 'Pate Cá Ngừ Thượng Hạng',
        price: '150.000đ',
    ...
    {
        image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuC2sDtZ49-NyIAdKu40HgElOtTvuZmwgkEOzbri0l2GuLa8O-5kzkeMvYGZ1UgBppUCYh_KZuhiiedn7Iah6L9FhCcy4tkhXN-KBKSVGcKgI2ZoIr9LbGn31FCkRzdbN0cSTRl2Vc0NhKU4fnbnj9_-1vIdDV4waonpNMtEFYrE0tvd-d956Nw4EgNypISySyzJ2lWohUTKKip0_r_MQmx_53xlnENbMo_0X9azaNAKbHRv1RFiFOTWIIMFEzqMYLTOG1twewIUqlfb',
        alt: 'Dog Toy',
        category: 'Đồ chơi',
    ...
    },
    {
        tag: 'Voucher',
        tagColor: 'bg-primary',
        image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuC7Wxtbn4zLBWG6YXT6oiw31WAdgaTgGmRp09KNDRpmTHfOLud41pg5u51pUqw9WzpAxw0lWFjXgatLby6yu-DIYN-_9GrJC22qevBYBVGa8fS3OCeVB-qpvh6t-N7JIQrB9aS-xPo9G76ubbj_trA_-YIZKWU1Y24w9ma1UJoT8UZPJ3luq8Qq-qFAG2LT91TADPE3OCD5Y9VLQqvJDGOU3nKauJFFemlldTv4nW08rtR0D2Kiv0l4cm5ZPyKLY4xkXMBZLK4K9K2s',
        alt: 'Spa Service',
        category: 'Dịch vụ Spa',
        name: 'Combo Tắm & Cắt Tỉa',
        price: '350.000đ',
        rating: '4.9',
        icon: 'calendar_month',
    },
    {
        tag: 'Hot',
        tagColor: 'bg-amber-500',
        image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAfuDWpp22fHyXXF9zayjnpcmPDPcC2UhofvzBEBX5npkp-Zs139LWAg4DOr6dab1ogFjwvCoyZE993cjN_9CJ9cuL35_-KOR7UDh-W38WYswEuZ3WsbKa4g77lbpZ4HriGtTELoAjFXvNLm05k7sCxtFJIqrWCWhCaaoXeJCJoviVxAuRupZe3OaBEzxdh7G97WQR9RhsZbsrofQrkS7cGO6EyjfW5XbtRWwSlfH0_L97w7r_36-SFf5XB0Dx9ZqqG6EwuQ-BGZz6A',
        alt: 'Pet Bed',
        category: 'Phụ kiện',
        name: 'Nệm Tròn Êm Ái',
        price: '420.000đ',
...
function CountdownTimer() {
    const [time, setTime] = useState({ h: 2, m: 45, s: 20 });
    useEffect(() => {
    ...
    return (
        <div className="flex items-center gap-4">
            <span className="text-sm text-slate-500 dark:text-slate-400">Kết thúc trong:</span>
            <div className="flex gap-2 text-center font-mono text-white">
                <div className="bg-slate-800 dark:bg-slate-700 rounded-md p-2 min-w-[40px]">
                    <span className="text-xl font-bold">{pad(time.h)}</span>
                </div>
                <span className="text-slate-800 dark:text-slate-400 text-xl py-2">:</span>
                <div className="bg-slate-800 dark:bg-slate-700 rounded-md p-2 min-w-[40px]">
                    <span className="text-xl font-bold">{pad(time.m)}</span>
                </div>
                <span className="text-slate-800 dark:text-slate-400 text-xl py-2">:</span>
                <div className="bg-primary rounded-md p-2 min-w-[40px]">
                    <span className="text-xl font-bold">{pad(time.s)}</span>
                </div>
            </div>
        </div>
    );
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {DEALS.map((deal) => (
                            <div
                                key={deal.name}
                                className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-200 dark:border-slate-700 group relative"
                            >
                                {deal.tag && (
                                    <span
                                        className={`absolute top-4 left-4 ${deal.tagColor} text-white text-xs font-bold px-2 py-1 rounded z-10`}
                                    >
                                        {deal.tag}
                                    </span>
                                )}
                                <div className="h-48 rounded-xl bg-slate-50 dark:bg-slate-800 mb-4 overflow-hidden flex items-center justify-center relative">
                                    <img
                                        alt={deal.alt}
                                        className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-500"
                                        src={deal.image}
                                    />
                                    <button className="absolute bottom-2 right-2 bg-white dark:bg-slate-700 p-2 rounded-full shadow-md text-primary opacity-0 group-hover:opacity-100 transition-opacity hover:bg-slate-100">
                                        <span className="material-symbols-outlined">{deal.icon}</span>
                                    </button>
                                </div>
                                <div className="space-y-2">
                                    <p className="text-xs text-slate-500">{deal.category}</p>
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center text-amber-400 text-xs gap-1">
                                            ...
                                            <span className="text-slate-400">({deal.rating})</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
