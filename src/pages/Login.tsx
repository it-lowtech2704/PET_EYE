...
export default function Login() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    login({ name: name || email.split('@')[0] || 'Khách hàng', email });
    navigate('/home');
  };

  const handleDemoLogin = () => {
    login({ name: 'Demo User', email: 'demo@carevia.vn' });
    navigate('/home');
  };
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-4 md:p-8 bg-slate-50 dark:bg-slate-900">
      ...
      >
        {/* Left: Illustration */}
        <div className="hidden md:flex md:w-1/2 bg-slate-100 dark:bg-slate-700 items-center justify-center p-12 relative overflow-hidden">
          ...
          <div className="absolute -top-20 -left-20 w-64 h-64 bg-[#1a2b4c]/5 rounded-full blur-3xl" />
        </div>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="flex flex-col gap-2">
              ...
              <input
                className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"
            ...
            <div className="flex flex-col gap-2">
              <div className="flex justify-between">
                ...
                <a className="text-xs font-semibold text-primary dark:text-blue-400 hover:underline" href="#">
                  Quên mật khẩu?
                </a>
              </div>
              <div className="relative">
                ...
                <input
                  className="w-full pl-12 pr-12 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"
                  placeholder="••••••••"
            ...
            <button
              type="submit"
              className="w-full py-4 bg-primary text-white font-semibold rounded-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all active:scale-[0.98]"
            >
              Đăng nhập ngay
            </button>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                type="button"
                onClick={handleDemoLogin}
                className="flex items-center justify-center gap-2 py-3 px-4 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
              >
          ...
          <div className="mt-10 text-center">
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Chưa có tài khoản?{' '}
              <Link to="/register" className="text-primary dark:text-blue-400 font-bold hover:underline ml-1">
                Đăng ký miễn phí
              </Link>
            </p>
          </div>
